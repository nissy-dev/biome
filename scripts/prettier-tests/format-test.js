...
async function biomeFormat(input, options) {
  ...
  const tmp_input = input.replaceAll("prettier-ignore" , "biome-ignore format:");
  fs.writeFileSync(tmpFilePath, tmp_input, "utf8");
  ...
  try {
    ...
    formatted = formatted.replaceAll("biome-ignore format:" , "prettier-ignore");
  } finally {
